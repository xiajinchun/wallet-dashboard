    ...
    },
    {
      ...
      children: [
        ...
        },
        {
          ...
          align: "center",
          render: (text, record) => (
            <a href={"https://explorer.zksync.io/address/" + record.address} target={"_blank"}>
              {text}
            </a>
          )
        },
    ...
    // },
    {
      ...
      render: (text) => (
        <Space>
          ...
          {text && text === "error" ? <Tag color="error">失败</Tag> : null}
        </Space>
      )
    },
    {
      ...
      render: (text, record) => (
        <Space>
          {record["result"] === "success" ? <Tag icon={<CheckCircleOutlined />} color="success"></Tag> : null}
          {record["result"] === "error" ? <Tag icon={<CloseCircleOutlined />} color="error"></Tag> : null}
          {record["result"] === "pending" ? <Tag icon={<SyncOutlined spin />} color="processing"></Tag> : null}
        </Space>
      )
    },
    {
      ...
      render: (text, record) => (
        <Space size="small">
          <Popconfirm
            title={t("zk_message_confirm_delete")}
            onConfirm={async () => {
  ...
  const addressDetailColumns = [
    {
      ...
      key: "logo",
      align: "center",
      render: (text, record) => <img src={"/protocol/" + record.id + ".png"} style={{ width: "20px", height: "20px" }} alt={record.id} />
    },
    {
      title: t("protocol"),
      dataIndex: "name",
      ...
      <Table.Summary>
        <Table.Summary.Row>
          ...
          <Table.Summary.Cell index={6}>
            <div style={centeredTextStyle}>
              <Text type="danger">{formatNumber(totalZkLiteEthBalance)}</Text>
            </div>
          </Table.Summary.Cell>
          ...
          <Table.Summary.Cell index={9}>
            <div style={centeredTextStyle}>
              <Text type="danger">{formatNumber(totalZkEraEthBalance)}</Text>
            </div>
          </Table.Summary.Cell>
          <Table.Summary.Cell index={10}>
            <div style={centeredTextStyle}>
              <Text type="danger">{formatNumber(totalZkEraUsdcBalance, 2)}</Text>
            </div>
          </Table.Summary.Cell>
          ...
          <Table.Summary.Cell index={15}>
            <div style={centeredTextStyle}>
              <Text type="danger">{formatNumber(totalL1Tol2Amount)}</Text>
            </div>
          </Table.Summary.Cell>
          <Table.Summary.Cell index={16}>
            <div style={centeredTextStyle}>
              <Text type="danger">{formatNumber(totalL2Tol1Amount)}</Text>
            </div>
          </Table.Summary.Cell>
          ...
          <Table.Summary.Cell index={21}>
            <div style={centeredTextStyle}>
              <Text type="danger">{formatNumber(totalAmount, 2)}</Text>
            </div>
          </Table.Summary.Cell>
          <Table.Summary.Cell index={22}>
            <div style={centeredTextStyle}>
              <Text type="danger">{formatNumber(totalFee, 2)}</Text>
            </div>
          </Table.Summary.Cell>
        ...
        <div className="zks_footer">
          <Card size={"small"} style={{ width: "300px" }}>
            ...
            >
              ...
              ></Button>
              <Popconfirm
                title={t("confirm_delete") + selectedKeys.length + t("address_count") + "?"}
                onConfirm={async () => {
                  await handleDeleteSelected();
                }}
              >
                <Button type="primary" danger size={"large"} style={{ width: "20%" }} icon={<DeleteOutlined />}></Button>
              </Popconfirm>
              <Button type="primary" icon={<DownloadOutlined />} size={"large"} style={{ width: "20%" }} onClick={exportToExcelFile} />
            </div>
          </Card>
        </div>
      </Content>
